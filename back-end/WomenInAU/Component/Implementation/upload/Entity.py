...
def submit_entity_function():
    ...
    eId = json_data['eId']
    if eId == 'none':
        add_entity(1, json_data, f,None)
    else:
        update_entity(1, json_data, f)
    return jsonify('true')
...
def save_entity_function():
    ...
    eId = json_data['eId']
    if eId == 'none':
        add_entity(0, json_data, f,None)
    else:
        update_entity(0, json_data, f)
    return jsonify('true')
...
def update_entity(func, json_data, f):
    eId = json_data['eId']
    print(eId)
    en = entityInfoDB.search_entity(eId)
    if en is None:
    ...
    if f != 0:
        ...
        imgLocation = str(en.eImage)
    neweId = add_entity(func, json_data, f,imgLocation)
    return neweId


# This entity has not been created before.
def add_entity(func, json_data, f,imgLoc):
    print(json_data)
    status = 0
    if func == 1:
        status = 1
